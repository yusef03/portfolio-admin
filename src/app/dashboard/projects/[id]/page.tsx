      ...
      {(['de', 'en', 'ar'] as Lang[]).map(l => (
        <button
          ...
          className={`px-3 py-1 text-xs font-medium transition-colors ${
            active === l ? 'bg-purple-600 text-white' : 'bg-gray-900 text-gray-400 hover:text-white'
          }`}
        >
          {l.toUpperCase()}
        </button>
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-1.5">
        {badges.map(b => (
          <span key={b} className="flex items-center gap-1 px-2 py-0.5 text-xs rounded-full bg-gray-800 border border-gray-700 text-gray-300">
            {b}
            <button
      ...
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 px-3 py-1.5 text-sm rounded-lg bg-gray-950 border border-gray-700 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500"
        />
        <button
          onClick={add}
    ...
    <div className="space-y-2">
      {features.map((f, i) => (
        <div key={i} className="flex gap-2 items-start">
          <span className="text-gray-500 text-xs pt-2 w-4 shrink-0">{i + 1}.</span>
          <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-2">
            <input
              ...
              placeholder="DE"
              className="px-2 py-1.5 text-xs rounded-lg bg-gray-950 border border-gray-700 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500"
            />
            <input
              type="text"
              value={f.en}
          ...
          <button
            ...
            className="text-gray-500 hover:text-red-400 transition-colors pt-1.5 shrink-0"
      ...
      <div className="flex gap-3 items-start">
        ...
        <div className="flex-1 space-y-2">
          <input
            ...
            className="w-full px-3 py-1.5 text-sm rounded-lg bg-gray-950 border border-gray-700 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 font-mono"
          />
          <div className="flex gap-2">
            <button
              onClick={() => inputRef.current?.click()}
              disabled={uploading}
              className="px-3 py-1.5 text-xs rounded-lg bg-gray-800 text-gray-300 hover:bg-gray-700 transition-colors disabled:opacity-50"
            >
            ...
            {imageUrl && (
              <button
                onClick={() => onChange('')}
                className="px-3 py-1.5 text-xs rounded-lg bg-gray-800 text-gray-400 hover:text-red-400 transition-colors"
              >
                Entfernen
              </button>
            )}
...
export default function ProjectEditorPage() {
  ...
  if (loading) {
    return <div className="text-gray-500 text-sm py-8 text-center">Lädt…</div>
  }
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-4">
        <h3 className="text-sm font-medium text-gray-400 uppercase tracking-wider">Basis</h3>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs text-gray-400 mb-1">
              Slug <span className="text-gray-600">(URL-Identifier)</span>
            </label>
            <input
              type="text"
              value={form.slug}
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">
              Titel <span className="text-gray-600">(Eigenname, kein i18n)</span>
            </label>
            <input
              type="text"
              value={form.title}
              onChange={e => set('title', e.target.value)}
              className="w-full px-3 py-2 text-sm rounded-lg bg-gray-950 border border-gray-700 text-white focus:outline-none focus:border-purple-500"
            />
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-medium text-gray-400 uppercase tracking-wider">Beschreibung</h3>
        ...
        <textarea
          ...
          className="w-full px-3 py-2 text-sm rounded-lg bg-gray-950 border border-gray-700 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 resize-y"
        />
      </div>

      {/* Badges */}
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3">
        <h3 className="text-sm font-medium text-gray-400 uppercase tracking-wider">Badges / Tech-Stack</h3>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3">
        <div>
          ...
          <p className="text-xs text-gray-600 mt-0.5">Hero-Bullets (DE / EN / AR je Zeile)</p>
        </div>
      ...
      {/* Bild */}
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-3">
        <h3 className="text-sm font-medium text-gray-400 uppercase tracking-wider">Bild</h3>
        <ImageUpload
      ...
      {/* Links */}
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-4">
        <h3 className="text-sm font-medium text-gray-400 uppercase tracking-wider">Links</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-1.5 text-sm rounded-lg bg-gray-950 border border-gray-700 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500"
            />
          </div>
          <div>
            <label className="block text-xs text-gray-400 mb-1">Demo-URL <span className="text-gray-600">(optional, noch nicht gerendert)</span></label>
            <input
              type="url"
              value={form.demo_url ?? ''}
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">Subseiten-URL <span className="text-gray-600">(optional)</span></label>
            <input
              type="text"
      ...
      {/* Meta */}
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 space-y-4">
        <h3 className="text-sm font-medium text-gray-400 uppercase tracking-wider">Meta</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs text-gray-400 mb-1">Zeitraum <span className="text-gray-600">(optional)</span></label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">Rolle <span className="text-gray-600">(optional)</span></label>
            <input
              type="text"
              value={form.role ?? ''}
          ...
          <div>
            <label className="block text-xs text-gray-400 mb-1">Sortierungs-Index</label>
            <input
              type="number"
        ...
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          />
          <span className="text-sm text-gray-300">Als Hero-Projekt auf der Startseite anzeigen ⭐</span>
        </label>
        {form.is_hero && (
